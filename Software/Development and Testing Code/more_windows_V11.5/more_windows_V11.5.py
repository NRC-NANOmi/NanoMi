...
class Plot_Controller(QWidget):
    def __init__(self):
        ...
        #set parameters for data outputs:
        self.diADC, self.diDAC = self.initAIOUSB()
        self.offset = 0
        self.span = 10
        DACSetBoardRange(self.diDAC, 0)
        self.channel = 0
    ...
    def show_pController(self):
        ...
        self.setWindowTitle('Lens 1 Setting')

        layout = QGridLayout()

        QToolTip.setFont(QFont('SansSerif', 80))
        ...
        # pushbutton to save number typed into line
        self.lbtn = QPushButton('Input Setting', self)
        self.lbtn.clicked.connect(self.on_click_input)
        layout.addWidget(self.lbtn)  
        
        # Title label for feedback display
        self.feedback_title = QLabel('Lens 1 Feedback (V):')
        ...
        # Label to display collected feedback from the board, updated once per second
        timer = QtCore.QTimer(self)
        self.feedback = QLabel('', self)
        timer.timeout.connect(lambda: self.feedback.setText(str(self.scan(0))))
        timer.start(1000)
    ...
    def on_click_input(self):
        ...
        # case where user does type store the number
            ...
            DACDirect(self.diDAC, self.channel, int((int(self.line_edit.text()) - self.offset) * 65535.0 / self.span))
            '''    
            # append to the list for saving to text file
    ...
    # function that reads inputs from AIOUSB board
    def scan(self, channel):
        ADC_SetCal(self.diADC, ":AUTO:")
        status = None
        data = None
        
        #do two scans because first scan is not useful
        for i in range(2):
            status, data = ADC_GetScanV(self.diADC)
        
        #reformat data to be readable by user
        data = ["{0:1.4f}".format(i) for i in data]
        
        return float(data[channel])
        
    # function that initializes AIOUSB boards
    def initAIOUSB(self):
        AIOUSB_Init()
        diADC = None
        diDAC = None
        ...
        for di in range(31):
            if 0 != devMask & (1 << di):
                ...
                #print("Board found at index {0} has deviceID {1:x} = {2}, SN= {3:x} [{4} dio bytes]".format(di, PID, name, sn, dio, ctrs))
                if PID == 0x815F:
                    diADC = di
                if PID == 0x8071:
                    diDAC = di
...
class Plot_Window(QtWidgets.QMainWindow):
    ...
    def show_plot_window(self):
        ...
        self.myFig = MyFigureCanvas(x_range=[-59,1], y_range=[0, 10], interval=1000)
...
class MyFigureCanvas(FigureCanvas, anim.FuncAnimation):
    ...
    def __init__(self, x_range:List, y_range:List, interval:int) -> None:
        ...
        self._ax_.set_ylabel('Lens 1 Feedback (V)')
    ...
    def _update_canvas_(self, i, y) -> None:
        ...
        '''
        y.append(round(self.scan(0), 2))     # Add new datapoint, channel = 0 here
