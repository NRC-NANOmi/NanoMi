...
#****************************************************************************************************************
 ...
 #In order to minimize the size of the entire UI, if the size of all lenses textboxes and widgets is higher than 400 (Height of all controller parameters textboxes and widgets), set it as the size of all the lenses.
        ...
        for i in range(number_of_lenses):
            ...
            self.widget.move(0,i*30+10)
            self.widget.resize(80,10)
            self.textboxes[i].move(0, i*30+20)
            self.textboxes[i].resize(50, 20)
            self.textboxes[i].textChanged.connect(lambda: self.updateDataSet())
            self.data["C"+str(i+1)] = self.textboxes[i]
        ...
        for i in range(number_of_lenses):
            self.wobble_buttons[i].move(50,i*30+20)
        ...
        self.textbox1.textChanged.connect(lambda: self.updateDataSet())
        self.data["Voltage increment frequency"] = self.textbox1
        ...
        self.textbox6.textChanged.connect(lambda: self.updateDataSet())
        self.data["Voltage increment value"] = self.textbox6
        ...
        self.textbox2.textChanged.connect(lambda: self.updateDataSet())
        self.data["Wobble frequency"] = self.textbox2
        ...
        self.textbox3.textChanged.connect(lambda: self.updateDataSet())
        self.data["Wobble percentage"] = self.textbox3
        ...
        self.textbox4.textChanged.connect(lambda: self.updateDataSet())
        self.data["Wobble duration"] = self.textbox4
        ...
        self.textbox5.textChanged.connect(lambda: self.updateDataSet())
        self.data["Wobble increment"] = self.textbox5
   ...
   #function to handle initialization - mainly calls a subfunction to create the user interface
    ...
    def setValue(self, name, value):
        for varName in self.data:
            if name in varName:
                self.data[varName].setText(value)
                return 0
        return -1
